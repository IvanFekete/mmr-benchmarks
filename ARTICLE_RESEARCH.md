...
The candidates are tested against three MMR distributions:

* *Uniform* - a standard uniform distribution U[0, 5000)
...
As a development host, I will use 1 vCPU Intel Xeon at 2.8 GHz, with 4 GB RAM. For this experiment, N will be scaled from 100,000 (mixed-workload percentile run) to 1,000,000 (isolated-op run).
...
Let’s check what happens when N changes: running the same isolated benchmarks at four scales with a uniform distribution:
...
Let’s check the p99/p50 ratio to analyze the behavior across different candidates. As you can see, Fenwick rank is 95/62 ≈ 1.5×, hybrid has similar numbers, while skip list rank is 1,500/600 ≈ 2.5×. The reason Fenwick’s distribution is tighter than the skip list’s is that Fenwick's access pattern is deterministic and doesn’t vary across different queries. Meanwhile, in the skip list, the level distribution randomizes the query’s depth, and forward pointer dereference time is also non-deterministic, so the slow operations are slower in both axes at once.
...
| N is tiny (under ~1,000 queued)                       | **Sorted array**     |
...
Read-heavy workloads with small ranges. If your queries are mostly reads within small ranges and writes occur rarely, the benefit of fast range processing diminishes, and constant time becomes more important. In this case, a pre-baked rank histogram would be faster than Fenwick, but the update cost (recalculating the entire histogram) will be relatively low because our system is read-heavy. I didn’t benchmark it because I specifically planned to focus on wide-range queries and frequent updates.

When you need garbage collection determinism. Hybrid’s `buckets [][]uint64` use slices that will change their size over time. The thing is, when the slice is shrinking, it can keep the backing array the same size, without actually using the reserved capacity and wasting memory. I have not seen this issue at my scale, but a hard-realtime system might experience increased memory usage (not a memory leak, but still something to avoid).

When the queue can’t fit in a single host. Past a certain shard size (say, 50M players), your queue can’t fit in a single host. The data structure choice within a single shard does not change, but cross-shard global rank queries become the hard problem. If you continue to use Fenwick, it will be hard to federate: you would maintain per-shard Fenwicks and aggregate at query time, which is fine for range counts but adds additional complexity for exact global rank (those results can’t be aggregated quickly).

And last, but not least: this is a single benchmark study and does not guarantee that my query mix will match yours. Production conditions may vary from case to case, so you should check them yourself before relying on these numbers. I have published the benchmark code, the workload generator, and the correctness oracle so you can re-run it with your own parameters.

## Reproducing the numbers

The project is available on my GitHub via the link. You can reproduce results, build the module, run the entry points, and get output using steps described in README.md.
...
8. Salvatore Sanfilippo (antirez). Redis source code and design notes for sorted sets. https://github.com/redis/redis/blob/unstable/src/t_zset.c
9. Ted Dunning. Computing extremely accurate quantiles using t-digests. arXiv:1902.04023, 2019.